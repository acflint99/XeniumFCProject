...
if (any(!args %in% valid_options)) {
  stop("Usage: Rscript scripts/Xenium_RL_Merge_Processing_res1.5.R [--dry-run|--overwrite]")
}
dry_run <- "--dry-run" %in% args
overwrite <- "--overwrite" %in% args
if (dry_run && overwrite) stop("--dry-run and --overwrite cannot be combined.")

plot_dir <- here(config$project$outputs_dir, "XenAld_RL_Res1.5_Plots")
rds_dir <- here(config$project$outputs_dir, "XenAld_RL_Res1.5_RDS")
merged_path <- here(
  config$project$outputs_dir, "XenAld_RL_Subsets_Res1.5_RDS", "Merged",
