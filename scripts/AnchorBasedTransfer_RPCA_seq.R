...
run_label_transfer <- function(sample_name,
  ...
  # Force everything to run in the current process
  plan(sequential) 
  # Increase the limit anyway, just in case a function still tries to check it
  ...
  rm(anchors) 
  gc() # Clear the heavy anchor object before starting the high-res plots
