...
master_obj <- readRDS(here("outputs", "XenAld_VZ_RDS", "Xenium_VZ_Integrated_newSubclusters.rds"))
...
updated_status <- future_lapply(sample_files, function(f) {
  ...
  # 3. Safe Injection
  temp_obj <- AddMetaData(temp_obj, metadata = relevant_labels, col.name = "subcluster")
  
  # 4. Success Check
  match_count <- sum(!is.na(temp_obj$subcluster))
