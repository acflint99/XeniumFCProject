...
if (any(!args %in% valid_options)) {
  stop("Usage: Rscript scripts/Xenium_VZ_Merge_res1.5.R [--dry-run|--overwrite]")
}
...
subset_dir <- file.path(output_root, "XenAld_VZ_Subsets_Res1.5_RDS")
...
output_path <- file.path(merged_dir, "Xenium_Merged_VZSubsets.rds")
manifest_path <- file.path(merged_dir, "Xenium_Merged_VZSubsets_manifest.csv")
...
if (dry_run) {
  cat("Expected VZ subset inputs:", length(subset_paths), "\n")
  cat("Existing VZ subset inputs:", sum(file.exists(subset_paths)), "\n")
