...
output_dir <- here("outputs", "XeniumAldingerABT_RLsubcluster_RDS")
if(!dir.exists(output_dir)) dir.create(output_dir)

# FIX: Load the object from the path provided
message("Loading master subclustered object...")
master_obj <- readRDS(here("outputs", "XenAld_RL_Integrated_RDS", "Xenium_RL_Integrated_newSubclusters.rds"))
...
updated_status <- future_lapply(sample_files, function(f) {
  ...
  out_path <- file.path(output_dir, paste0(s_name, "_Ald_RL.rds"))
