...
generate_spatial_reports <- function(sample_id, input_rds_path, base_plot_dir) {
  ...
  obj$subcluster <- factor(obj$subcluster, levels = subcluster_order)
  ...
  save_highlight <- function(seurat_obj, targets, title_suffix, filename) {
    ...
    seurat_obj[[group_name]] <- ifelse(
      seurat_obj$subcluster %in% targets,
      as.character(seurat_obj$subcluster),
      "Other"
    )
  ...
  # B. Group Highlights
  save_highlight(obj, c("VZP", "Migrating PCs", "Immature PCs", "Differentiated PCs", "Mature PCs"),
                 "Purkinje_Lineage", paste0(sample_id, "_Purkinje_Spatial.png"))
  save_highlight(obj, c("VZP", "RG Progenitors", "Prolif RG", "BG", "Astrocytes"),
                 "Glia_Lineage", paste0(sample_id, "_Glia_Spatial.png"))
  save_highlight(obj, c("VZP", "GABA Progenitors", "Golgi Cells", "GABA Interneurons", "MLI"),
                 "GABA_Lineage", paste0(sample_id, "_GABA_Spatial.png"))

  #C. Faceted Spatial
  coords <- GetTissueCoordinates(obj, type = "Xenium")
  plot_data <- cbind(coords, cluster = obj$subcluster)
  design <- "ABCDE\nFGHI#\nJKLM#\nNOP##"
  ...
  # D. Marker DotPlot
  rev_levels <- factor(as.character(obj$subcluster), levels = rev(subcluster_order))
  names(rev_levels) <- Cells(obj)
  obj <- AddMetaData(obj, metadata = rev_levels, col.name = "subcluster_rev")

  Idents(obj) <- "subcluster_rev"
  obj_plot <- subset(obj, idents = subcluster_order)
