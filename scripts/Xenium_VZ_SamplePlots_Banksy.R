...
generate_spatial_reports <- function(sample_id, input_rds_path, base_plot_dir) {
  ...
  obj$Banksy_VZ_Refined <- factor(obj$Banksy_VZ_Refined, levels = vz_B_subcluster_order)
  
  # Export Cluster Statistics
  stats <- obj@meta.data %>%
    group_by(Banksy_VZ_Refined) %>%
  ...
  save_highlight <- function(seurat_obj, targets, title_suffix, filename) {
    ...
    seurat_obj[[group_name]] <- ifelse(
      seurat_obj$Banksy_VZ_Refined %in% targets,
      as.character(seurat_obj$Banksy_VZ_Refined),
      "Other"
    )
    ...
    current_cols <- c("Other" = bg_color, vz_B_palette[targets])
  ...
  # B. Group Highlights
  save_highlight(obj, c( "PC Progenitors", "Maturing PCs", "Migratory PCs", "Intermediate PCs", "Rostral PCs", "Mature PCs"),
                 "Purkinje_Lineage", paste0(sample_id, "_Purkinje_Spatial.png"))
  save_highlight(obj, c("Glial Progenitors", "Bergmann Glia", "Astrocytes", "Ependymal Cells"),
                 "Glia_Lineage", paste0(sample_id, "_Glia_Spatial.png"))
  save_highlight(obj, c("GABAergic Progenitors", "GABAergic DCN", "Glutamatergic DCN"),
                 "GABA_Lineage", paste0(sample_id, "_GABA_Spatial.png"))

  #C. Faceted Spatial
  coords <- GetTissueCoordinates(obj, type = "Xenium")
  plot_data <- cbind(coords, cluster = obj$Banksy_VZ_Refined)
  design <- "ABC#\nDEF#\nGHI#\nKLMN\nOP##"
  ...
  p_facet <- plot_data %>%
    ...
    scale_color_manual(values = vz_B_palette) +
    scale_y_reverse() + 
    coord_fixed() + 
    theme_void() +
    theme(
  ...
  # D. Marker DotPlot
  rev_levels <- factor(as.character(obj$Banksy_VZ_Refined), levels = rev(vz_B_subcluster_order))
  names(rev_levels) <- Cells(obj)
  obj <- AddMetaData(obj, metadata = rev_levels, col.name = "vz_B_subcluster_rev")

  Idents(obj) <- "vz_B_subcluster_rev"
  obj_plot <- subset(obj, idents = vz_B_subcluster_order)

  p_dot <- DotPlot(obj_plot, features = vz_B_markers, cols = c("lightgrey", "red"),
