...
Cairo::CairoTIFF(
  filename = file.path(plot_dir, paste0("XenSepp_VZ_CorrPlot.tif")),
  width = 10,
  height = 10,
  units = "in",
  res = 600
...
print(p1)
grDevices::dev.off()
ggplot2::ggsave(file.path(plot_dir, "XenSepp_VZ_CorrPlot.pdf"), p1,
...
for (clust in cluster_names) {
  ...
  print(p) 
  Cairo::CairoTIFF(
    filename = file.path(plot_dir, paste0("XenSepp_VZ_", clust, "_Sign_UMAP.tif")),
    width = 10,
    height = 10,
    units = "in",
    res = 600
...
# Save the plot
Cairo::CairoTIFF(
  filename = file.path(plot_dir, "XenSepp_VZ_Signature_Heatmap.tif"),
  width = 12,
  height = 10,
  units = "in",
  res = 600
...
print(p_sig)
grDevices::dev.off()
ggplot2::ggsave(file.path(plot_dir, "XenSepp_VZ_Signature_Heatmap.pdf"), p_sig,
