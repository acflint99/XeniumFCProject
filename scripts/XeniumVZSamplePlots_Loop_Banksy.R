...
source(here("scripts", "Xenium_VZ_SamplePlots_Banksy.R"))
source(here("scripts", "color_palette.R"))

# 3. Define Samples (run one line (3-4 samples) at a time)
samples_to_run <- c(
  "FB330_1_X_G", "FB78_X_G", "GZFB_12_X_G_1"
...
# 3. Define Paths
input_base <- here("outputs", "XeniumAldingerABT_Banksy_VZsubcluster_RDS")
output_base <- here("outputs", "XeniumAldingerABT_Banksy_VZsubcluster_Results")
...
for (s in samples_to_run) {
  ...
  input_path <- file.path(input_base, paste0(s, "_Banksy_VZ.rds"))
