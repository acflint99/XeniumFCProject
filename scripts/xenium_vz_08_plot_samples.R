...
if (length(task_args) > 1L) {
  stop("Usage: Rscript scripts/xenium_vz_08_plot_samples.R [--list|--dry-run|--overwrite] [TASK_ID]")
}
...
output_base <- file.path(output_root, "xenium", "vz", "08_sample_reports")
...
if (dry_run) {
  compact_dry_run(
    paste0("VZ report task ", task_id, "/", length(sample_ids), " [", sample_id, "]"),
    inputs = input_path,
...
source(here("scripts", "xenium_vz_plot_sample.R"))
